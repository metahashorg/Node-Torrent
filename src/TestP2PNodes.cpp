...
static void sendTests(Curl::CurlInstance &libevent, const std::map<std::string, std::vector<TestResult>> &results, const PrivateKey &privateKey, const std::string &myIp, const std::map<std::string, std::string> &nodesForTest, const std::string &resultServer, size_t size) {
    ...
    const std::string response = Curl::request(libevent, resultServer, toSend, "", "", 10);
    ...
    while (true) {
        try {
            while (true) {
                ...
                Curl::CurlInstance libevent = Curl::getInstance();
