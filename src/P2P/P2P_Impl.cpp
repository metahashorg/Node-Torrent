...
std::vector<Segment> P2P_Impl::makeSegments(size_t countSegments, size_t size, size_t minSize) {
...
P2P_Impl::P2P_Impl(size_t countThreads) {
    for (size_t i = 0; i < countThreads; i++) {
        threads.emplace_back(blockedQueue);
    }
}

std::string P2P_Impl::request(const CurlInstance &curl, const std::string& qs, const std::string& postData, const std::string& header, const std::string& server) {
...
SendAllResult P2P_Impl::process(const std::vector<std::reference_wrapper<const std::string>> &requestServers, const std::string &qs, const std::string &post, const std::string &header, const RequestFunctionSimple &requestFunction) {
    SendAllResult result;
    std::mutex mutResult;
    
    parallelFor(8, requestServers.begin(), requestServers.end(), [&result, &mutResult, &qs, &post, &header, &requestFunction](const std::string &server) {
        Timer tt;
        ResponseParse r;
        try {
            r.response = requestFunction(qs, post, header, server);
        ...
        result.results.emplace_back(server, r, tt.count());
    });
