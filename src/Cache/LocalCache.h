...
using LocalCacheElementBalance = LocalCacheElement<BalanceInfo>;
...
class LocalCacheBalance: public LocalCacheInternal<LocalCacheElementBalance> {
public:
    
    LocalCacheBalance(size_t maxCountElements)
        : LocalCacheInternal<LocalCacheElementBalance>(maxCountElements)
    {}
    
    void setBalance(const common::HashedString &address, const BalanceInfo &balance);
    
    void replaceBalanceAfterUncle(const common::HashedString &address, const BalanceInfo &balance);
    
    ~LocalCacheBalance() override = default;
};

class LocalCache {
public:
    
    using GetterBalance = std::function<BalanceInfo(const std::string &address, const std::string &setName)>;
...
public:
    ...
    LocalCache(size_t maxCountElements)
        ...
        , localCacheBalance(maxCountElements)
    {}
    
    void setFunctions(const GetterBalance &getterBalance_, const GetterTxs &getterTxs_, size_t countThreads);
...
public:
    ...
    LocalCacheBalance localCacheBalance;
...
private:
    ...
    std::function<BalanceInfo(const std::string &address)> getterBalance2;
