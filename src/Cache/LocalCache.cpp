...
ReturnT makeNewElementValue(const T &value, bool isSaveFull) {
    if constexpr(std::is_same_v<T, BalanceInfo>) {
        (void)isSaveFull;
        return value;
    } else if constexpr (std::is_same_v<T, std::vector<TransactionInfo>>) {
        if (isSaveFull) {
            return value;
        } else {
...
void LocalCacheBalance::setBalance(const HashedString& address, const BalanceInfo& balance) {
    std::lock_guard<std::shared_mutex> lock(mut);
    auto found = cache.find(address);
    if (found == cache.end()) {
        return;
    }
    found->second.value = balance;
}
...
void LocalCacheBalance::replaceBalanceAfterUncle(const HashedString& address, const BalanceInfo& balance) {
    // TODO не приспособлена под удаление из кэша
    std::lock_guard<std::shared_mutex> lock(mut);
    auto &element = cache.at(address);
...
void LocalCache::setFunctions(const GetterBalance& getterBalance_, const GetterTxs& getterTxs_, size_t countThreads) {
    getterBalance = getterBalance_;
...
void LocalCache::setAddressPathAndLoad(const std::string& path, const std::function<std::string(std::string)> &processAddress, const std::string &setName) {
    ...
    if (!addressesFile.empty()) {
        ...
        getterBalance2 = std::bind(getterBalance, _1, std::ref(setName));
...
void LocalCache::addAddresses(const std::unordered_set<std::string> &newAddressesStrs, const std::string &group) {
    ...
    for (const HashedString &address: newAddresses) {
        ...
        localCacheBalance.addIfNoExist(address, getterBalance2, false, false);
    }
...
void LocalCache::processFile() {
    ...
    parallelFor(countThreadsForLoad, addressesInFile.begin(), addressesInFile.end(), [this](const HashedString &address) {
        ...
        localCacheBalance.addIfNoExist(address, getterBalance2, false, true);
    });
...
void LocalCache::setLastBlockNum(size_t blockNum) {
    localCacheBalance.setLastBlockNum(blockNum);
    localCacheTxs.setLastBlockNum(blockNum);
}

template class LocalCacheInternal<LocalCacheElementBalance>;
template class LocalCacheInternal<LocalCacheElementTxs>;
template class LocalCacheInternal<LocalCacheElementTxsStatus>;

template void LocalCacheInternal<LocalCacheElementBalance>::addElement<BalanceInfo>(const HashedString &address, const BalanceInfo &value, bool isSyncBlockchainThreadUpdated);
